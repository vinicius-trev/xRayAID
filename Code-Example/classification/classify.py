...
class Classify:
    """
    Class that realizes a classification for one image
    Build and create the model for classification and heatmap

    Args: 
        model: CheXNet Model
        session: TF main session
        layerName: Layer that you want the output weights for heatmap production
        classsIdx: Class index, in our case, since it only has one class the idx is 0
    """
    ...
    """
    Attempt to find the final convolution layer in the network
    ...
    """
    Execute inference for both models, first to get the last convolution
    outbputs for the heatmap and other to get the classification probabilities
    ...
    Returns:
        Heatmap values and classification probabilie
    """
